...
# Don't need if we use NIR, not much CB for cool stars
...
def BIS_inject_spectral_active(wavelength, flux, bis_coeffs_up, bis_coeffs_low, dep_cut_faculae, vel_cut_faculae, velo_shift_faculae, scaling_factor):
    ...
    for kk in np.arange(flux.size):
        if (flux[kk] < dep_cut_faculae) and (flux[kk] > 0.5):
            ...
            rv_shift = (np.poly1d(bis_coeffs_up)(
                flux[kk])+velo_shift_faculae)*scaling_factor
            wav_shift = Delta_wavelength(rv_shift, wavelength[kk])
            wave_new[kk] = wavelength[kk] + wav_shift

        elif (flux[kk] <= 0.5):

            rv_shift = (np.poly1d(bis_coeffs_low)(
                flux[kk])+velo_shift_faculae)*scaling_factor
            wav_shift = Delta_wavelength(rv_shift, wavelength[kk])
...
def Line_removal(wavelength, spec_seed_spot):
    ...
    line_list = np.array(
        [5250.2084, 5250.6453, 5434.5232, 6173.3344, 6301.5008])
    ...
    for ids, lines in enumerate(line_list):
        ...
        spectrum, depth = bisector_measurement_line(
            wavelength[index], spec_seed_spot[index], spec_seed_spot[index]*0+10**-3, lines)
    ...
    for i_s in np.arange(bis_size):
        ...
        index_c = (depths_s > 0.2+i_s*step_size_BIS) & (depths_s <
                                                        0.2+(i_s+1)*step_size_BIS)
        if np.argwhere(index_c).size > 0:
            vel_arr_cut.append(np.mean(vels_cut_s[index_c]))
            dep_arr_cut.append(np.mean(depth_cut_s[index_c]))

    bis_coeffs_s = np.polyfit(np.array(dep_arr_cut), np.array(
        vel_arr_cut), deg=2, rcond=None, full=False, w=None, cov=False)
...
for i_teff in np.arange(tstar_array.size):
    ...
    if Temp <= 4300:
        mask_template = input_prefix + \
            config.get('data_io', 'mask_prefix')+'K6_mask.txt'
    elif Temp > 4300:
        mask_template = input_prefix + \
    ...
    for i_t in np.arange(T_eff.size):
        ...
        out_name = output_dir+'/new_noconti_convec_T_eff_' + \
            str(int(T_eff[i_t]))+'.bin'
        seed_cube.tofile(out_name)

        out_name1 = output_dir+'/new_conti_convec_T_eff_' + \
            str(int(T_eff[i_t]))+'.bin'
